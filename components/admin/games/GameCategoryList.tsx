...
export function GameCategoryList({ gameId, projectId }: GameCategoryListProps) {
  ...
  const fetchAvailableCategories = async () => {
    try {
      ...
      if (data.data) {
        // 过滤掉已添加的分类
        const existingCategoryIds = categories.map(gc => gc.categoryId)
  ...
  // 更新分类状态
  const handleToggleActive = async (category: GameCategory) => {
    try {
  ...
  const handleDelete = async (categoryId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/projects/${projectId}/games/${gameId}/categories?categoryId=${categoryId}`,
        {
          method: 'DELETE',
        }
      )
